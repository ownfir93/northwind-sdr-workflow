...
// prompts sent to n8n. Prompts/logic stay in git; n8n stays dumb.
//
...
// Flat reference text (what n8n actually receives, appended after the instructions).
const appended = (...files: string[]) =>
  "\n\n---\n# REFERENCE\n\n" + files.map(read).join("\n\n---\n\n");

// Builds the flat system (instructions + references) for n8n AND the structured display for the UI.
function buildGen(o: {
...
export function accountResearchPrompt(vars: { account: string }): GenInput {
  const system =
    "You are an account research agent for Northwind (composable CDP / Reverse ETL / warehouse-native " +
    ...
    "Northwind activation use case, likely personas, and risks.\n" +
